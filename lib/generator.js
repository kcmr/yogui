...
module.exports = class Generator {
  ...
  get isSingleGenerator() {
    return this.generators.length === 1;
  }
  ...
  get promptForDestiny() {
    return {
      type: 'input',
      name: 'dest',
      message: 'Destiny',
      default: this.currentDirectoryName
    };
  }
  ...
  async _getCurrentGenerator() {
    if (this.isSingleGenerator) {
      return {
        type: this.generators[0]
      }
    }

    return await this._prompt(
      this._getPromptForUserGeneratorTypes()
    );
  }
