...
module.exports = class Generator {
  ...
  _askQuestionsForGenerator() {
    ...
    if (!this.generator.dest) {
      questions.concat(this._promptForDestiny);
    }
  ...
  _generate(params) {
    ...
    const relativePath = scaffoldParams.dest
      .replace(process.cwd(), '')
      .replace(path.sep, '');
  ...
  _getDestinyPath(params) {
    let destiny = (this.generator.dest)
      ? this._replaceConfigVars(this.generator.dest, params)
      : params.dest;

    if (path.basename(destiny) === this._currentDirectoryName) {
      destiny = '.';
    }

    return path.resolve(process.cwd(), destiny);
  }

  /**
   * Replaces variables like {{name}} by the value of
   * the corresponding key in params object (params[name]).
   * @param {String} string string to transform.
   * @param {Object} params question params.
   * @returns {String} string with the replacements.
   */
  _replaceConfigVars(string, params) {
    const varRegex = /{{(\w)+}}/g;
    const matches = string.match(varRegex);

    if (matches) {
      return string.replace(varRegex, match => {
  ...
  _getFileReplacement(params) {
    ...
    if (fileNameReplacement) {
      ...
      return [
        scaffoldFileName, 
        this._replaceConfigVars(variable, params)
      ];
    }
  }
};
